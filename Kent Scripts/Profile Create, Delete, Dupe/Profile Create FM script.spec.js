...
test('New Profile FM', async ({page}) => {
    ...
    console.log('Create New FM Profile')
    ...
    await test.step('Check for existing profile', async () => {
        ...
        await page.getByRole('textbox', { name: 'Filter by name' }).fill('kent qa fm');
        ...
        const existing = await page.getByRole('cell',{name: 'kent qa fm', exact:true}).isVisible()
        ...
        await page.getByRole('textbox', { name: '* Name' }).fill('kent qa fm');
        await page.getByRole('radio', { name: 'Fully-managed For company-' }).check();
        await page.getByRole('button', { name: 'OK' }).click();
