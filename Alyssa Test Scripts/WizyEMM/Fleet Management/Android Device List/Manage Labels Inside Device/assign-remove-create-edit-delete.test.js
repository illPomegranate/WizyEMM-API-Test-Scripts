...
//Open Fleet Management Module to reveal tabs - Manually click Fleet Management if its not responding 
  ...
  await page.getByRole('textbox', { name: 'Serial Number' }).fill('gardenvilla18');
  await page.getByRole('textbox', { name: 'Serial Number' }).press('Enter');

  let androidDeviceExists = false;
  try {
    const rowLocator = page.getByRole('row', { name: 'gardenvilla18 wizyEMM-Villa Manila' });
  ...
  await page.getByRole('link', { name: 'gardenvilla18' }).click();
  await page.waitForTimeout(2000);

  //Click Assign - If its not quickly responding, manually click to the side upon assigning label to move forward to 'OK' button
  ...
  await page.waitForTimeout(10000); //Long timeout due to 'Action Summary' banner
  ...
  const registerResponse = await registerDevice(page, {
    serialNumber: 'gardenvilla18',
    profile: 'fm',
    customData: 'AdminTest',
    name: 'wizyEMM-Villa',
