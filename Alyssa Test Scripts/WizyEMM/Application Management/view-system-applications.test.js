...
test('System Applications', async ({ page }) => {
...
//Open Application Management Module to reveal tabs - Manually click Application Management if its not responding 
  ...
  const locator = page.getByRole('link', { name: 'System Applications' });
  const systemApp = await logLinkResponse(page, locator, '/api/v1/systemapps', 'System Applications');
  expect(systemApp.status()).toBe(200);
  expect(systemApp).toBeTruthy();
